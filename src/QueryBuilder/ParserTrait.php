...
trait ParserTrait
{
     ...
     *
     * @param  \Turahe\Core\Filter\Filter  $rule
     */
     ...
     * @param  \Turahe\Core\Filters\Filter  $filter
     * @return \Turahe\Core\Date\Carbon|array
     */
     ...
     *
     * @param  \Turahe\Core\Filters\Filter&\Turahe\Core\Filters\CountableRelation  $filter
     * @param  \stdClass  $rule
    ...
    {
        if (count($value) !== 2) {
            throw new QueryBuilderException(Str::title($rule->query->rule).' must be an array with only two items.');
        } elseif ($value[1] == '') {
            throw new QueryBuilderException('Please select the between value for '.Str::title($rule->query->rule).' filter.');
        }
